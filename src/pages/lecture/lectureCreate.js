import { ArrowBack } from '@mui/icons-material'
import React from 'react'
import MainLayout from '../../components/MainLayout'
import Grid2 from '@mui/material/Unstable_Grid2/Grid2'
import { Box, Button, Divider, Typography } from '@mui/material'
import { Link } from 'react-router-dom'
import LectureForm from '../../components/LectureForm'
import { createLecture } from '../../services/lectureService'
import { showAlert, showLoading } from '../../utils/swal'

const LectureCreate = () => {

  const handleLectureCreate = async (data) => {
    showLoading()
    try {
        const lectureData = await createLecture(data)
        console.log(lectureData)
        showAlert("New Lecture Created",`New lecture created successfully.`, "success")
    } catch (error) {
        console.error(`Error lecture create:`, error)
        showAlert("Create Lecture Failed",error, "error")
    }
}

  return (
    <MainLayout>
      <Grid2 container spacing={2}>
        <Grid2 xs={12} md={8}>
          <Typography variant='h5' component='h1'>Add New Lecture</Typography>
        </Grid2>
        <Grid2 xs={12} md={4} sx={{
          textAlign: 'end'
        }}>
          <Button component={Link} to="/lectures" variant="contained" endIcon={<ArrowBack />}>
            Back
          </Button>
        </Grid2>
      </Grid2>
      <Divider sx={{ my: 3 }} />
      <Box padding={2}>
        <LectureForm onSubmit={handleLectureCreate} />
      </Box>
    </MainLayout>
  )
}

export default LectureCreate